...
async function loadJsonFile(filePath: string): Promise<Record<string, unknown> | null> {
  try {
    ...
    throw new GhostCommentError(
      GhostCommentErrorType.CONFIG_ERROR,
 ...
 */
async function findConfigFile(workingDirectory: string, customPath?: string): Promise<string | null> {
  if (customPath) {
    ...
    try {
      ...
      throw new GhostCommentError(
        GhostCommentErrorType.CONFIG_ERROR,
        `Custom config file not found: ${resolvedPath}`
      );
    }
  }
 ...
 */
async function loadConfigFile(workingDirectory: string, customPath?: string): Promise<ConfigFile> {
 ...
 */
async function loadPackageJsonConfig(workingDirectory: string): Promise<ConfigFile> {
  ...
  if (typeof config !== 'object' || Array.isArray(config)) {
    throw new GhostCommentError(
      GhostCommentErrorType.CONFIG_ERROR,
      `package.json ${PACKAGE_JSON_FIELD} field must be an object`
    );
  }
...
function loadEnvironmentConfig(): EnvironmentConfig {
  ...
  if (process.env.GITLAB_TOKEN) env.GITLAB_TOKEN = process.env.GITLAB_TOKEN;
  if (process.env.GITLAB_URL) env.GITLAB_URL = process.env.GITLAB_URL;
  
  // GhostComment-specific environment variables
  if (process.env.GC_CONFIG_PATH) env.GC_CONFIG_PATH = process.env.GC_CONFIG_PATH;
  if (process.env.GC_REPO_OWNER) env.GC_REPO_OWNER = process.env.GC_REPO_OWNER;
  if (process.env.GC_REPO_NAME) env.GC_REPO_NAME = process.env.GC_REPO_NAME;
  if (process.env.GC_PULL_NUMBER) env.GC_PULL_NUMBER = process.env.GC_PULL_NUMBER;
  if (process.env.GC_DEBUG) env.GC_DEBUG = process.env.GC_DEBUG;
  if (process.env.GC_DRY_RUN) env.GC_DRY_RUN = process.env.GC_DRY_RUN;
 ...
 */
function detectGitHubActions(): boolean {
 ...
 */
function loadGitHubActionsContext(): Partial<EnvironmentConfig> {
  if (!detectGitHubActions()) {
    return {};
  }
  
  const context: Partial<EnvironmentConfig> = {};
 ...
 */
function validateConfig(config: Partial<GhostCommentConfig>): void {
  const errors: string[] = [];
 ...
 */
function mergeConfigs(...configs: Array<Partial<GhostCommentConfig>>): GhostCommentConfig {
  const merged: Partial<GhostCommentConfig> = {};
  
  // Merge in reverse priority order (last config has highest priority)
  for (const config of configs) {
    if (config.prefix !== undefined) merged.prefix = config.prefix;
    if (config.include !== undefined) merged.include = [...config.include];
    if (config.exclude !== undefined) merged.exclude = [...config.exclude];
    if (config.failOnFound !== undefined) merged.failOnFound = config.failOnFound;
    if (config.githubToken !== undefined) merged.githubToken = config.githubToken;
    if (config.gitlabToken !== undefined) merged.gitlabToken = config.gitlabToken;
    if (config.gitlabUrl !== undefined) merged.gitlabUrl = config.gitlabUrl;
  }
 ...
 */
function environmentToConfig(env: EnvironmentConfig): Partial<GhostCommentConfig> {
  const config: Partial<GhostCommentConfig> = {};
  
  if (env.GITHUB_TOKEN) config.githubToken = env.GITHUB_TOKEN;
  if (env.GITLAB_TOKEN) config.gitlabToken = env.GITLAB_TOKEN;
  if (env.GITLAB_URL) config.gitlabUrl = env.GITLAB_URL;
 ...
 */
function cliOptionsToConfig(options: CLIOptions): Partial<GhostCommentConfig> {
  const config: Partial<GhostCommentConfig> = {};
  
  if (options.token) config.githubToken = options.token;
  if (options.failOnFound !== undefined) config.failOnFound = options.failOnFound;
 ...
 */
export function validateConfigOnly(config: Partial<GhostCommentConfig>): {
  valid: boolean;
  errors: string[];
} {
  try {
...
export async function createDefaultConfigFile(
  workingDirectory: string = process.cwd(),
  fileName: string = CONFIG_FILE_NAMES[0]
): Promise<string> {

  ...
  try {
    ...
    if (stats.size > FILE_LIMITS.MAX_FILE_SIZE) {
      throw new GhostCommentError(
        ...
        `File ${filePath} is too large (${stats.size} bytes, max: ${FILE_LIMITS.MAX_FILE_SIZE})`
      );
    }
  } catch (error) {
      ...
      if (match && match[1]) {
        ...
        if (commentContent.length > CONFIG_VALIDATION.MAX_COMMENT_LENGTH) {
          throw new GhostCommentError(
            ...
            `Comment too long at ${filePath}:${index + 1} (${commentContent.length} chars, max: ${CONFIG_VALIDATION.MAX_COMMENT_LENGTH})`
          );
        }
 ...
 */
function validateScanConfig(config: GhostCommentConfig): void {
  if (!config.prefix) {
    throw new GhostCommentError(
      GhostCommentErrorType.CONFIG_ERROR,
      'Comment prefix is required'
    );
  }
  ...
  if (config.prefix.length > CONFIG_VALIDATION.MAX_PREFIX_LENGTH) {
    throw new GhostCommentError(
      GhostCommentErrorType.CONFIG_ERROR,
      `Prefix too long: ${config.prefix} (max: ${CONFIG_VALIDATION.MAX_PREFIX_LENGTH})`
    );
  }
  ...
  if (config.include.length > CONFIG_VALIDATION.MAX_INCLUDE_PATTERNS) {
    throw new GhostCommentError(
      GhostCommentErrorType.CONFIG_ERROR,
      `Too many include patterns: ${config.include.length} (max: ${CONFIG_VALIDATION.MAX_INCLUDE_PATTERNS})`
    );
  }
  ...
  if (config.exclude && config.exclude.length > CONFIG_VALIDATION.MAX_EXCLUDE_PATTERNS) {
    throw new GhostCommentError(
      GhostCommentErrorType.CONFIG_ERROR,
      `Too many exclude patterns: ${config.exclude.length} (max: ${CONFIG_VALIDATION.MAX_EXCLUDE_PATTERNS})`
    );
  }
}
...
export async function scanFiles(
  ...
  try {
    ...
    if (files.length > FILE_LIMITS.MAX_FILES) {
      throw new GhostCommentError(
        GhostCommentErrorType.FILE_ERROR,
        `Too many files to process: ${files.length} (max: ${FILE_LIMITS.MAX_FILES}). Consider refining your include/exclude patterns.`
      );
    }
    ...
    const allComments: GhostComment[] = [];
    
    for (let i = 0; i < files.length; i += concurrency) {
      const batch = files.slice(i, i + concurrency);
      
      const batchPromises = batch.map(filePath =>
        scanFile(filePath, config, workingDirectory)
      );
    ...
    throw new GhostCommentError(
      GhostCommentErrorType.FILE_ERROR,
      'Failed to scan files for ghost comments',
      error as Error
    );
  }
}
...
export async function scanSingleFile(
  ...
  try {
    ...
    throw new GhostCommentError(
      GhostCommentErrorType.FILE_ERROR,
      `Failed to scan single file: ${filePath}`,
      error as Error
    );
  }
}
 ...
 */
export async function countGhostComments(
  config: GhostCommentConfig,
  workingDirectory: string = process.cwd()
): Promise<number> {
  validateScanConfig(config);
  ...
  try {
    ...
    if (files.length > FILE_LIMITS.MAX_FILES) {
      throw new GhostCommentError(
        GhostCommentErrorType.FILE_ERROR,
        `Too many files to process: ${files.length} (max: ${FILE_LIMITS.MAX_FILES})`
      );
    }
    ...
    throw new GhostCommentError(
      GhostCommentErrorType.FILE_ERROR,
      'Failed to count ghost comments',
      error as Error
    );
  }
}
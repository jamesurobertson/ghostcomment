...
export class GitHubClient {
   ...
   */
  async getPullRequest(context: GitContext): Promise<{
    number: number;
    head: { sha: string };
    base: { sha: string };
  }> {
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 404) {
          throw new GhostCommentError(
            GhostCommentErrorType.GITHUB_API_ERROR,
            `Pull request #${context.pullNumber} not found in ${context.owner}/${context.repo}`
          );
        }
      }
      throw new GhostCommentError(
        GhostCommentErrorType.GITHUB_API_ERROR,
        `Failed to get pull request information`,
        error as Error
      );
    }
  }
  ...
  private async postSingleComment(
    ...
    try {
      ...
      const response = await this.client.post(
        `/repos/${context.owner}/${context.repo}/pulls/${context.pullNumber}/comments`,
        requestBody
      );

      return response.data as GitHubCommentResponse;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response?.status === 403) {
          ...
          if (resetTime || retryAfter) {
            throw new GhostCommentError(
              ...
              `GitHub API rate limit exceeded. Reset at: ${resetTime || 'unknown'}`
            );
          }
        }
    ...
    for (let i = 0; i < apiComments.length; i++) {
      ...
      try {
        ...
        if (error instanceof GhostCommentError) {
          ...
          console.error(`✗ Failed to post comment on ${apiComment.filePath}:${apiComment.lineNumber}: ${(error as Error).message}`);
        }
      }
   ...
   */
  async listReviewComments(context: GitContext): Promise<GitHubCommentResponse[]> {
    try {
      const response = await this.client.get(
        `/repos/${context.owner}/${context.repo}/pulls/${context.pullNumber}/comments`
      );

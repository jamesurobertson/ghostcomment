...
export class GitLabClient {
  ...
  constructor(config: GitLabClientConfig) {
    ...
    if (!this.config.token) {
      throw new GhostCommentError(
        GhostCommentErrorType.AUTH_ERROR,
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response?.status === 403) {
          throw new GhostCommentError(
            GhostCommentErrorType.AUTH_ERROR,
            'GitLab token does not have sufficient permissions'
          );
        }
      }
      throw new GhostCommentError(
        GhostCommentErrorType.NETWORK_ERROR,
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 404) {
          throw new GhostCommentError(
            GhostCommentErrorType.GITLAB_API_ERROR,
            `Project ${owner}/${repo} not found`
          );
        }
      }
      throw new GhostCommentError(
        GhostCommentErrorType.GITLAB_API_ERROR,
        `Failed to get project ID for ${owner}/${repo}`,
        error as Error
      );
    }
  }
   ...
   */
  async getMergeRequest(context: GitContext): Promise<GitLabMergeRequest> {
    try {
      const projectId = await this.getProjectId(context.owner, context.repo);
      
      const response = await this.client.get(
        `/projects/${projectId}/merge_requests/${context.pullNumber}`
      );
      ...
      if (axios.isAxiosError(error)) {
        if (error.response?.status === 404) {
          throw new GhostCommentError(
            GhostCommentErrorType.GITLAB_API_ERROR,
            `Merge request !${context.pullNumber} not found in ${context.owner}/${context.repo}`
          );
        }
      }
      throw new GhostCommentError(
        GhostCommentErrorType.GITLAB_API_ERROR,
        `Failed to get merge request information`,
        error as Error
      );
    }
  }
   ...
   */
  private createPosition(comment: APIComment): GitLabPosition {
    return {
   ...
   */
  private async postSingleDiscussion(
    comment: APIComment,
    projectId: number,
    mrIid: number,
    attempt: number = 1
  ): Promise<GitLabDiscussionResponse> {
    try {
      const requestBody: {
        body: string;
      ...
      return response.data as GitLabDiscussionResponse;
    } catch (error) {
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.response?.status === 400) {
          ...
          if (errorMessage.includes('line') || errorMessage.includes('position')) {
            throw new GhostCommentError(
              GhostCommentErrorType.GITLAB_API_ERROR,
              `Line ${comment.lineNumber} not in diff for ${comment.filePath}`,
              undefined
            );
          }
        }
        ...
        if (axiosError.response?.status === 429) {
          throw new GhostCommentError(
            GhostCommentErrorType.RATE_LIMIT_ERROR,
        ...
        if (axiosError.response?.status === 403) {
          throw new GhostCommentError(
            GhostCommentErrorType.AUTH_ERROR,
            'Insufficient permissions to post to this merge request'
          );
        }
      ...
      throw new GhostCommentError(
        GhostCommentErrorType.NETWORK_ERROR,
        'Network error while posting discussion',
        error as Error
      );
    }
  }
   ...
   */
  private convertToAPIComments(comments: GhostComment[]): APIComment[] {
  ...
  async postDiscussions(
    ...
    for (let i = 0; i < apiComments.length; i++) {
      ...
      try {
        const response = await this.postSingleDiscussion(
          apiComment,
          mrInfo.project_id,
          mrInfo.iid
        );
        ...
        if (error instanceof GhostCommentError) {
          ...
          console.error(`✗ Failed to post discussion on ${apiComment.filePath}:${apiComment.lineNumber}: ${(error as Error).message}`);
        }
      }
   ...
   */
  async listDiscussions(context: GitContext): Promise<GitLabDiscussionResponse[]> {
    try {
      const projectId = await this.getProjectId(context.owner, context.repo);
      
      const response = await this.client.get(
        `/projects/${projectId}/merge_requests/${context.pullNumber}/discussions`
      );
      ...
      throw new GhostCommentError(
        GhostCommentErrorType.GITLAB_API_ERROR,
        'Failed to list merge request discussions',
        error as Error
      );
    }
  }
